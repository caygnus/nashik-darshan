...
type Secret struct {
	...
	// Type of secret: private_key or publishable_key
	...
	// Array of permissions (read, write, etc.)
...
func (*Secret) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case secret.FieldMetadata, secret.FieldPermissions:
			values[i] = new([]byte)
		case secret.FieldID, secret.FieldStatus, secret.FieldCreatedBy, secret.FieldUpdatedBy, secret.FieldName, secret.FieldType, secret.FieldProvider, secret.FieldValue, secret.FieldPrefix:
	...
	for i := range columns {
		...
		case secret.FieldType:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.Type = types.SecretType(value.String)
			}
		case secret.FieldProvider:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field provider", values[i])
			} else if value.Valid {
				_m.Provider = types.SecretProvider(value.String)
			}
