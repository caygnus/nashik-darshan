import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for DtoPlaceResponse
void main() {
  final instance = DtoPlaceResponseBuilder();
  // TODO add properties to the builder and call build()

  group(DtoPlaceResponse, () {
    // BuiltMap<String, String> address
    test('to test the property `address`', () async {
      // TODO
    });

    // BuiltList<String> amenities
    test('to test the property `amenities`', () async {
      // TODO
    });

    // BuiltList<String> categories
    test('to test the property `categories`', () async {
      // TODO
    });

    // String createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // String createdBy
    test('to test the property `createdBy`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // BuiltList<DtoPlaceImageResponse> images
    test('to test the property `images`', () async {
      // TODO
    });

    // String lastViewedAt
    test('to test the property `lastViewedAt`', () async {
      // TODO
    });

    // TypesLocation location
    test('to test the property `location`', () async {
      // TODO
    });

    // String longDescription
    test('to test the property `longDescription`', () async {
      // TODO
    });

    // String placeType
    test('to test the property `placeType`', () async {
      // TODO
    });

    // num popularityScore
    test('to test the property `popularityScore`', () async {
      // TODO
    });

    // String primaryImageUrl
    test('to test the property `primaryImageUrl`', () async {
      // TODO
    });

    // num ratingAvg
    test('to test the property `ratingAvg`', () async {
      // TODO
    });

    // int ratingCount
    test('to test the property `ratingCount`', () async {
      // TODO
    });

    // String shortDescription
    test('to test the property `shortDescription`', () async {
      // TODO
    });

    // String slug
    test('to test the property `slug`', () async {
      // TODO
    });

    // TypesStatus status
    test('to test the property `status`', () async {
      // TODO
    });

    // String subtitle
    test('to test the property `subtitle`', () async {
      // TODO
    });

    // String thumbnailUrl
    test('to test the property `thumbnailUrl`', () async {
      // TODO
    });

    // String title
    test('to test the property `title`', () async {
      // TODO
    });

    // String updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // String updatedBy
    test('to test the property `updatedBy`', () async {
      // TODO
    });

    // Engagement fields for feed functionality
    // int viewCount
    test('to test the property `viewCount`', () async {
      // TODO
    });

  });
}
