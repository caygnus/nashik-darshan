import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for DtoReviewResponse
void main() {
  final instance = DtoReviewResponseBuilder();
  // TODO add properties to the builder and call build()

  group(DtoReviewResponse, () {
    // String content
    test('to test the property `content`', () async {
      // TODO
    });

    // String createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // String createdBy
    test('to test the property `createdBy`', () async {
      // TODO
    });

    // String entityId
    test('to test the property `entityId`', () async {
      // TODO
    });

    // TypesReviewEntityType entityType
    test('to test the property `entityType`', () async {
      // TODO
    });

    // int helpfulCount
    test('to test the property `helpfulCount`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // BuiltList<String> images
    test('to test the property `images`', () async {
      // TODO
    });

    // bool isFeatured
    test('to test the property `isFeatured`', () async {
      // TODO
    });

    // bool isVerified
    test('to test the property `isVerified`', () async {
      // TODO
    });

    // int notHelpfulCount
    test('to test the property `notHelpfulCount`', () async {
      // TODO
    });

    // num rating
    test('to test the property `rating`', () async {
      // TODO
    });

    // TypesStatus status
    test('to test the property `status`', () async {
      // TODO
    });

    // BuiltList<String> tags
    test('to test the property `tags`', () async {
      // TODO
    });

    // String title
    test('to test the property `title`', () async {
      // TODO
    });

    // String updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // String updatedBy
    test('to test the property `updatedBy`', () async {
      // TODO
    });

    // String userId
    test('to test the property `userId`', () async {
      // TODO
    });

  });
}
